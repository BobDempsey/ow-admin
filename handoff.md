...
`2a5adc0` (a test fix after the dialog icons). Commit `handoff.md` and `tasks.md` edits after
each task.
...
The app is live at https://ow.bobdempsey83.com (also https://ow-admin-five.vercel.app), deployed on 2026-09-16 from this working copy with `npx vercel@latest deploy --prod --yes --scope bobdempseys-projects`. There is no git remote, so nothing deploys on push; run that command again to ship a change.
