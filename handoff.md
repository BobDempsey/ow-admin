...
`accessibility`, and `about-page` (51 requirements total). The change that created them is at
...
`openspec/changes/add-settings-dialog/` is active: all 15 tasks done, committed in `05d0a93`, not archived. It adds a `settings-dialog` capability, exempts Settings in `admin-navigation`'s "Placeholder nav entries", and rewrites `accessibility`'s "No drag-only interactions" to allow opt-in column dragging. What it built:
...
`openspec/changes/add-theme-switcher/` is also active: all 14 tasks done, committed in `916c1cc`, not archived. It adds a new `theme-switcher` capability and a "Contrast in both themes" scenario to `accessibility`'s "Sufficient color contrast". What it built:
...
- The user added three optional tasks to `tasks.md` on 2026-09-13: a light,
  ...
  gotchas).
- `add-settings-dialog` is committed in `05d0a93` but not archived; archive it after `add-theme-switcher`.
