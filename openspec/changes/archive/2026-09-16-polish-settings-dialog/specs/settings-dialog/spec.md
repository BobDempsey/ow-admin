...
The user list screen SHALL show a "Table settings" button on the same row
as the Search users field, placed across from it, and on narrow screens
the button SHALL wrap below the field rather than scroll sideways.
Activating the button SHALL open a modal dialog titled "Table settings"
without changing the URL. The button SHALL be exposed to assistive
technology as a button that opens a dialog. No other screen SHALL offer
the dialog.
...
- **THEN** the Table settings button is on the same row as the Search users
  field, at the opposite end of the row
...
**Reason**: The dialog holds only table settings, so it opens from the user list, and the Settings nav entry returns to a placeholder.
**Migration**: Use the Table settings button on the user list; see "Open table settings from the user list".

### Requirement: Theme setting
**Reason**: It repeated the header Theme control, and the dialog now holds only table settings.
**Migration**: Choose Light, Dark or System in the header Theme control.
