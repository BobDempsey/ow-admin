...
| 2.5.7 Dragging Movements | AA | Passes | With default settings grid columns cannot be moved or resized, so no function needs dragging (`e2e/grid.e2e.ts`). Both were drag-only before this audit. Turning on Draggable columns in Settings makes column reordering drag-only; the dialog names the failure beside the setting, and it is listed under "Known gaps". Resizing stays off either way. |
...
With default settings, none of the level A or AA criteria above are known gaps. Three things are still open:

- **2.5.7 Dragging Movements, user list, while Draggable columns is on.** An admin can turn on Draggable columns in Settings, and columns can then be reordered only by dragging, with no keyboard or single-pointer alternative. The setting is off by default, and the dialog names this failure beside it.
...
- **Column dragging is a setting that fails 2.5.7.** AG Grid Community offers only dragging to move or resize columns. Resizing stays off, and columns flex to fill the width. Moving is off by default; an admin who wants it can turn on Draggable columns in Settings, which says beside the setting that it fails 2.5.7. A keyboard way to move columns was not built.
