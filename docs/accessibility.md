...
**Header row at 320 px.** The browser suite covers the icon buttons, the X Close buttons, the dark header and the open AI assistant drawer. One check still fails: at 320 px the Menu button wraps onto a second header row (see "Known gaps").
...
| 1.3.2 Meaningful Sequence | A | Passes | DOM order matches visual order; AG Grid runs with `ensureDomOrder`. Keyboard flows in `e2e/keyboard.e2e.ts` follow reading order. The header reads wordmark, nav entries, AI assistant, Theme, then Menu below 768 px, in DOM order, and the Table settings dialog's Close sits right after its heading in both DOM and visual order (`e2e/keyboard.e2e.ts`, `e2e/settings.e2e.ts`). At 320 px the Menu button currently wraps below the wordmark, so its visual position no longer follows Theme on the same row; see "Known gaps". |
...
| 1.4.10 Reflow | AA | Passes | No page-level sideways scroll at 320 px on any screen or the dialog (`e2e/layout.e2e.ts`). At 320 px the nav entries move into a drawer, and the open nav drawer and AI assistant drawer fit the viewport (`e2e/layout.e2e.ts`). The header does not scroll sideways, but it now takes two rows: the wordmark, AI assistant and Theme on the first and Menu on the second, which the one-row check in `e2e/layout.e2e.ts` reports as a failure (see "Known gaps"). The grid scrolls sideways inside its own box, which the criterion allows for data tables. The Actions column scrolls with the grid, and a row's Actions menu stays inside the viewport and clear of its button for the last row at 320 px (`e2e/row-actions.e2e.ts`). Below 1024 px the detail screen's Password and Demo cards follow the form in one column (`e2e/save-bar.e2e.ts`). |
...
- **The header takes two rows at 320 px.** The wordmark, AI assistant, Theme and Menu need about 293 px, and the header row has 288 px inside its padding, so Menu wraps below the wordmark. The page does not scroll sideways, so 1.4.10 still passes, but the one-row check in `e2e/layout.e2e.ts` fails until the row fits again.
