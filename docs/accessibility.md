...
| 2.4.3 Focus Order | A | Passes | After navigation focus moves to the screen's `h1`; after a failed submit to the first invalid field; after the dialog closes to Save; after Try again to the heading; on opening Table settings to the dialog's heading, and on closing it back to the Table settings button; the Theme menu opens on its checked item and returns focus to the Theme button when it closes (`e2e/keyboard.e2e.ts`, `e2e/settings.e2e.ts`, `e2e/theme.e2e.ts`, unit tests). |
...
| 2.4.7 Focus Visible | AA | Passes | Every control shows a solid 2 px outline; grid cells and headers show a solid 3 px ring. Theme menu items draw their outline inside their own edge, so it is never clipped by the menu. Rings are sky-700 in light and sky-400 in dark. |
